...
from .configuration_whisper_openai_encoder import WhisperConfig
...
class AudioEncoder(PreTrainedModel):
    def __init__(self, config: WhisperConfig):
        ...
        n_mels, n_ctx, n_state, n_head, n_layer = config.n_mels, config.n_ctx, config.n_state, config.n_head, config.n_layer
    ...
    def forward(self, x: Tensor, padding_mask: Tensor=None):
        ...
        input_positional_embedding = self.positional_embedding[:x.size(1)]  # TODO: 验证维度
        assert x.shape[1:] == input_positional_embedding.shape, "incorrect audio shape"
        x = (x + input_positional_embedding).to(x.dtype)
        if padding_mask is not None:  # TODO: 验证计算
            bsz = x.size(0)
