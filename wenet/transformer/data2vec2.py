...
from wenet.transformer.base import Modality

from wenet.transformer.base import (
    MaskSeed,
    D2vModalityConfig,
    ModalitySpecificEncoder,
    get_annealed_rate,
)
from wenet.transformer.modules import (
    D2vDecoderConfig,
    AltBlock,
    Decoder1d,
)

from wenet.transformer.audio import (
    D2vAudioConfig,
    AudioEncoder,
)
...
class Data2VecMultiModel(BaseFairseqModel):
    ...
    def __init__(self, cfg: Data2VecMultiConfig, modalities, skip_ema=False, task=None):
        ...
        mod_cfg = getattr(cfg.modalities, self.modalities.lower())
        conf = {}
        for k,v in mod_cfg.items():
            if k in D2vAudioConfig.__dataclass_fields__.keys():
                conf[k] = v
        mod_cfg = D2vAudioConfig(**conf)

        mod_cfg.decoder = None
        ...
        self.modality_encoders[self.modalities] = enc
    ...
    def infer(self, source, padding_mask):
        x, masked_padding_mask, alibi_bias = self.modality_encoders['AUDIO'].infer(
            source,
            padding_mask
        )
